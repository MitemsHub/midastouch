"""RETIRED — tests for the [3b] LV broker view (VPS era). The subject is gone.

**Retired 2026-09-20.** `morning_status._print_lv_broker_view` no longer exists: the
LV arm was one of the seven non-trading surfaces deleted by commit `4fba1fe`, and the
broker-snapshot view went with it (the VPS-era marker is retired per
`docs/GOLD_SURFACE_RETIREMENT_20260919.md`). Ten tests against a deleted function
cannot be fixed, only repointed at their subject — and their subject has no successor
with these semantics: the live gold arm's evidence view is `_print_midas_arm`, which
reads the EA's own ledger rather than an MT5 broker snapshot, and it is pinned by
`test_midas_telemetry.py`, `test_midas_clock.py`, `test_morning_status_preset.py` and
`test_midas_correlation.py`.

The module skips (rather than deletes) so the retirement is visible in every run and
the description below survives as the record of what was pinned. If a broker-snapshot
view is ever reintroduced for this account, these are the contracts it must meet —
and the skip reason is the pointer to them.

ORIGINAL PINS (kept as the record):
...
pytest.skip(
    "RETIRED: tests the deleted morning_status._print_lv_broker_view (LV arm removed "
    "by commit 4fba1fe; see this module's docstring for the live equivalents)",
