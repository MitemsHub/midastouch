...
# --------------------------------------------------------------------------- #
# The low-volatility signal mask
# --------------------------------------------------------------------------- #

def test_the_low_vol_mask_suppresses_only_the_quiet_bars() -> None:
    """The filter is a SIGNAL mask, and it must be judged on the engine's own median.

    `signal_mask` is passed into `gold_walkforward.simulate`, not applied to the finished
    trade list: a suppressed signal has to free its slot, or the reported sequence is one
    the EA could not have traded. The mask's boundary is the -0.3522R/trade cell, i.e. ATR
    below 0.8x its trailing median.
    """
    import numpy as np
    look = gg.gw.ATR_LOOKBACK
    n = look + 100
    atr = np.full(n, 10.0)
