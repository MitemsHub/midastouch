...
def _shared_corpus_data():
    """One build for both engines over the certified micro corpus.
    ...
    """
    d = p6.build_data(str(REPO / "data" / "forex" / "xauusd" / "XAUUSDmicro_H1.csv"),
                      str(REPO / "data" / "forex" / "xauusd" / "XAUUSDmicro_M15.csv"), 900)
    for k in p6.KS:
        arr = [0] * len(d["close"])
        for i in range(len(arr)):
            arr[i] = ms.bb_touch(d["close"], i, 20, k)
        d["bb"][k] = arr
    h1 = ms.load_bars(str(REPO / "data" / "forex" / "xauusd" / "XAUUSDmicro_H1.csv"))
    m15 = ms.load_bars(str(REPO / "data" / "forex" / "xauusd" / "XAUUSDmicro_M15.csv"))
    h4 = ms.h4_series(h1)
