...
def certify(equity0: float, blocked_hours: set[int] | None = None, *,
            ...
            exit_mode: str = "touch",
            spike_overshoot_r: float = 0.251,
            no_auto_disable: bool = False,
            risk_frac: float = 0.005,
            use_bandfade: bool = True,          # lab knob: harness-faithful default; preset runs BF OFF
    ...
    def regime_at(i):
        ...
        sep_min = MIN_EMA_SEP if h1_sep is None else h1_sep
        if hF[j] > hM[j] > hS[j] and c > hF[j] and sep >= sep_min:
            return BULL, pct
        if hF[j] < hM[j] < hS[j] and c < hF[j] and sep >= sep_min:
            return BEAR, pct
        return RANGE, pct
    ...
    for i in range(i_lo, i_hi):
        ...
        if pos and i >= pos["entry_i"]:
            p = pos
            ladder = exit_mode.startswith("ladder")
            spike = exit_mode == "ladder_spike"
            if p["dir"] < 0:
                hit_sl, hit_tp = b["h"] >= p["sl"], b["l"] <= p["tp"]
                open_sl = ladder and b["o"] >= p["sl"]
                open_tp = ladder and b["o"] <= p["tp"]
            else:
                hit_sl, hit_tp = b["l"] <= p["sl"], b["h"] >= p["tp"]
                open_sl = ladder and b["o"] <= p["sl"]
                open_tp = ladder and b["o"] >= p["tp"]
            closed = False
            if open_sl or (hit_sl and not hit_tp):
                ...
                # ladder mode (docs/HYBRID_EXIT_STUDY.md): bar-open ladder fill at
                # the OPEN price (gap-through) when the open is beyond the stop;
                # otherwise spike-path fill at the stop level (clean in ladder,
                # paper-measured overshoot in ladder_spike).
                if open_sl:
                    r = p["dir"] * (b["o"] - p["entry"]) / p["sd"] + p["r_extra"]
                elif ladder:
                    fill = p["sl"] - p["dir"] * (spike_overshoot_r if spike else 0.0) * p["sd"]
                    r = p["dir"] * (fill - p["entry"]) / p["sd"] + p["r_extra"]
                else:
                ...
                if open_tp:
                    # engine ladder: TP honored only at bar-open; fills at the open
                    # (favorable overshoot rides to the open price)
                    r = p["dir"] * (b["o"] - p["entry"]) / p["sd"] + p["r_extra"]
                    reason = "TP"; closed = True
                elif ladder and i == p["entry_i"]:
                    # entry-bar TP touch: spike-path fill at the TP level (conservative)
                    r = p["tp_r"] + p["r_extra"]; reason = "TP"; closed = True
                elif ladder:
                    pass  # intrabar TP touch after entry bar: the bar-open ladder
                          # cannot see it; the position rides to the next open
                else:
                    r = p["tp_r"] + p["r_extra"]; reason = "TP"; closed = True
            elif hit_sl and hit_tp:
                # both levels touched: ladder fills SL-first at the open when the
                # open is beyond the stop; otherwise spike-path SL (conservative)
                if open_sl:
                    r = p["dir"] * (b["o"] - p["entry"]) / p["sd"] + p["r_extra"]
                elif ladder:
                    fill = p["sl"] - p["dir"] * (spike_overshoot_r if spike else 0.0) * p["sd"]
            ...
            if closed:
                ...
                led["n"] += 1; led["r"] += r
                if (not no_auto_disable and t["strat"] not in disabled
                        and led["n"] >= AUTO_DISABLE_N
                        and led["r"] / led["n"] < 0.0):
        ...
        if reg in (BULL, BEAR) and rsi[i] is not None:
            ...
            if lo_pb <= pb <= hi_pb:
                ...
                if d < 0 and not eF[i] < eM[i]:
                    ok = False
                if m15_full_stack:
                    if d > 0 and not (eF[i] > eM[i] > eS[i]):
                        ok = False
                    if d < 0 and not (eF[i] < eM[i] < eS[i]):
                        ok = False
                if mom_confirm and not any(n == "MOM" and d == d for n, d, _ in legs):
                    ok = False
                if ok:
        ...
        risk_money = eq * risk_frac   # VSG: risk input is a study variable; 0.005 = certified path
...
def main():
    ...
    ap.add_argument("--exit-mode", choices=["touch", "ladder", "ladder_spike"],
                    default="touch",
                    help="touch = first-touch fills (hybrid/ideal); ladder = engine "
                         "bar-open ladder, gap-through fills, no intrabar TP after "
                         "entry bar (ideal spike fills); ladder_spike = ladder + "
                         "paper-measured spike overshoot (docs/HYBRID_EXIT_STUDY.md)")
    ap.add_argument("--spike-overshoot-r", type=float, default=0.251)
    ap.add_argument("--risk-frac", type=float, default=0.005,
                    help="risk fraction of equity per trade (VSG V1 lever; 0.005 = certified)")
    ap.add_argument("--no-auto-disable", action="store_true",
                    help="fill-cost diagnostics only: hold the strategy book open so "
                         "cost scenarios share one trade set (default off = certified)")
    ...
    rep = certify(args.equity, blocks, stop_mult=args.stop_mult,
                  ...
                  spike_overshoot_r=args.spike_overshoot_r,
                  risk_frac=args.risk_frac,
                  no_auto_disable=args.no_auto_disable)
    rep["blocked_hours"] = sorted(blocks)
    rep["spec"] = spec_block(tp_mult=args.tp_mult, stop_mult=args.stop_mult,
                             min_score_bonus=args.min_score_bonus)
    rep["exit_mode"] = args.exit_mode
    rep["risk_frac"] = args.risk_frac
    rep["no_auto_disable"] = args.no_auto_disable
    if args.exit_mode.startswith("ladder"):
        rep["exit_mode_note"] = "engine bar-open ladder mirror: stop/TP evaluated at "\
            "bar open only, fills at open price (gap-through), no intrabar TP touch "\
            "after entry bar, spike-path stop fills" + (
                f" at +{args.spike_overshoot_r}R overshoot" if args.exit_mode == "ladder_spike"
                else " at clean stop level (idealized)")
