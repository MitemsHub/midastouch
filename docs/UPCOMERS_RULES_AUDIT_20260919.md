...
| Attempt | Result |
|---|---|
| `/login:1293766 /password:<REDACTED> /server:Upcomers-Server` (read-only account, **no special characters** in the password) | args confirmed **intact** in the process command line; terminal logged **nothing** |
...
> **Server:** `Upcomers-Server` · **Login:** `1293766` · **Password:** `<REDACTED — rotate this; it was committed to the indices repo's history on 2026-09-19>`
...
> pass. `src/midas_prop/risk/upcomers_rules.py` therefore refuses to assume
...
`docs/UPCOMERS_INSTRUMENT_SELECTION_20260919.md`,
`src/midas_prop/risk/upcomers_rules.py`,
