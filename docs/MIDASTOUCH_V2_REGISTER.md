...
| P6 | **Consistent-daily-income study** (REGISTERED 2026-09-18 on operator directive — "the bot must be active and consistent: many small wins per day, compounding does the scaling; e.g. $0.45 × 66 = $30/day") | The two axes that govern fill density and daily smoothness were never swept: entry timeframe (M5 vs M15) and TP shape (1.0/1.5/1.8/2.0 × stop distance). Pre-registered BEFORE any result existed: ranking = max(positive-days %, median daily PnL) subject to n ≥ 60, pf ≥ 1.15, net_r > 0; every M5 config must ALSO survive an Apr→Sep OOS leg (Jan–Mar declared in-sample); NO-SHIP if nothing survives both. | Research-only engine (`scripts/midas_p6_income_study.py`) reusing the certified primitives with a parameterized timeframe and TP multiple; M5 corpus fetched from the broker (50,000 bars 2026-01-06→09-18, provenance `artifacts/midas_p6_m5_fetch.json`). | **EXECUTED 2026-09-18** (artifact `midas_p6_income_study_20260918.json`): 36 configs, 32 eligible full-span; after the OOS law killed 10 of 12 M5 rows, the winner is **M5 entries, k=2.0, TP=1.5R** — full-span 57.2% positive days, median $6.41/day (on the $5,000 study book), pf 1.368, n=263; OOS alone n=176, pf 1.150, 52.9% positive days, median $2.94/day. Sole OOS survivor alongside it: M5 k=2.0 TP=1.8R (pf 1.199). Cross-check on the current M15 config (k=2.0): the median day declines monotonically as TP lengthens — 1.0R +$4.29, 1.5R +$2.91, 1.8R −$2.48, 2.0R (the live shape) −$2.50 — so the live TP shape is the worst daily-income shape of the four, and every shortened TP beats it. The amendment targets 1.5R (not 1.0R) because 1.0R missed the M5 OOS pf gate (1.138 < 1.15) while 1.5R survived it; on M15 both are positive-median-day improvements over live. | **Amendment path (queued behind the 2026-10-01 reading):** an M5-entry LV variant is NEW EA code (M5 bar tracking + signal path), NOT telemetry-only — it requires a new version with full §1 parity treatment and is therefore a post-reading amendment, pre-registered now. Interim SIA-class item for the same reading: TP 2.0R→1.5R on the existing M15 config (the measured best median-day M15 shape). Kill rules for any M5 trial arm: trailing 20-trade expectancy < −0.30R, or two consecutive full-loss days, reverts the arm to the certified M15 config.
...
Apr→Sep OOS leg (pf 1.150) where 10 of 12 M5 configs failed; M15 evidence
independently confirms TP 1.5R as the best median-day shape on the current
config. Amendment (LV M5 variant + TP change) is QUEUED behind the
2026-10-01 reading per the row's amendment path — no live/paper arm changes
tonight from this result.
